...
import { ShieldCheck, AlertCircle, Save, Edit2, X, Clock, Activity } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white shadow-lg rounded-lg overflow-hidden">
          ...
          {showStatusHistory && profile?.statusHistory && (
                ...
                {profile.statusHistory.map((history, index) => (
                  <div key={index} className="flex items-start space-x-3 text-sm">
                    <div className={`flex-shrink-0 w-2 h-2 mt-2 rounded-full ${
                      history.status === 'active' ? 'bg-green-500' : 'bg-red-500'
                    }`} />
                    <div>
                      ...
                      {history.reason && (
                        <div className="text-gray-600 mt-1">{history.reason}</div>
                      )}
                    </div>
                  </div>
                ))}
